...
myString = "Autofrettage_Analysis"
...
#myID_5 = Create_Datum_Plane_by_Principal(XYPLANE,myPart,myString,2.0)
...
#Create_Analysis_Step(myString,"Step-1","Initial",0.1,0.1,1E-015,300,ON)
Create_Analysis_Step(myString,"Step-1","Initial",0.1,0.1,1E-05,300,ON)

Create_Boundary_Condition_by_Instance(myString,"Cylinder-1","Set-RP-1","BC-Set-RP-1","Initial",SET,SET,SET,UNSET,UNSET,UNSET)
Create_Boundary_Condition_by_Instance(myString,"Cylinder-1","Set-RP-2","BC-Set-RP-2","Initial",UNSET,UNSET,SET,SET,SET,UNSET)
...
# Create_Boundary_Condition_by_Instance(myString,"Cylinder-1","SPLA_Point","BC-Imperfection","Step-1",-myThickness,UNSET,UNSET,UNSET,UNSET,UNSET)
# Deactivate_BC(myString,"BC-Imperfection","Step-2")
#Create_SPLA(myString,"Cylinder-1","SPLA_Point","Perturbation_Load","Step-1",-1000.0)
Create_Pressure_Load(myString,"Cylinder-1","Internal_Pressure","Step-1","Internal_Surface",200)



#Create_Composite_Layup(myString,myPart,"Cylinder_3D","Quasi_Isotropic",myPlyNumber,"CFRP",myThickness/myPlyNumber,myAngle)
Create_Isotropic_Section(myString,"Alu_section","Alu")
Assign_Isotropic_Material(myString,myPart,"Cylinder_3D","Alu_section")

#CreateCutout(myString,myPart,7.5,myID_4,myEdge,0.0,0.0,myLength/2.0)

Create_Mesh_Solid(myString,myPart,Mesh_Size)
myFace = Create_Set_Face(myRadius,0.0,myLength/2.0,myString,myPart,"Outer_Surface")
AssignStack(myString,myPart,myFace)

CreateJob(myString,"Alu_Cylinder",8)
