...
def Create_Material_Data_2D(model,material_name,e11,e22,nu12,g12,g13,g23,density):
    mdb.models[model].Material(name=material_name)
...
def Create_Material_Data_2D_Ductile_Damage(model,material_name,e11,e22,nu12,g12,g13,g23,density,Y,FS,D_at_F):
    mdb.models[model].Material(name=material_name)
    mdb.models[model].materials[material_name].Elastic(type=LAMINA, table=((e11,e22,nu12,g12,g13,g23), ))
    mdb.models[model].materials[material_name].Density(table=((density, ), ))
    mdb.models[model].materials[material_name].Plastic(table=((Y, 0.0), ))
    mdb.models[model].materials[material_name].DuctileDamageInitiation(table=((FS, 0.0, 0.0), ))
    mdb.models[model].materials[material_name].ductileDamageInitiation.DamageEvolution(type=DISPLACEMENT, table=((D_at_F, ), ))
#------------------------------------------------------------------------------
...
def Create_Dynamic_Analysis_Step(model,step_name,pre_step_name,Initial_inc,Max_inc,Min_inc,Inc_Number,NL_ON_OFF,period):
    a = mdb.models[model].ImplicitDynamicsStep(name=step_name, previous=pre_step_name, timePeriod=period, maxNumInc=Inc_Number, initialInc=Initial_inc, minInc=Min_inc, nlgeom=ON)
    a = mdb.models[model].fieldOutputRequests['F-Output-1'].setValues(frequency=1)
    a = mdb.models[model].fieldOutputRequests['F-Output-1'].setValues(variables=('S', 'PE', 'PEEQ', 'PEMAG', 'LE', 'U', 'V', 'A', 'RF', 'CF', 'P', 'CSTRESS', 'CDISP', 'STATUS'))
    a = mdb.models[model].historyOutputRequests['H-Output-1'].setValues(frequency=1)
...
myPart = "Cylinder"
myString = str(myPart)+"_Explosion"
...
Create_Material_Data_2D_Ductile_Damage(myString,"Steel",myE11,myE22,myNu12,myG12,myG13,myG23,myDensity,myYield,0.001,0.4)


Create_Assembly(myString,myPart,"Cylinder-1")


Create_Dynamic_Analysis_Step(myString,"Step-1","Initial",0.002649,1,1E-015,100000,ON,0.0045)
...
Create_Composite_Layup_2D(myString,myPart,"Cylinder_2D","An_Isotropic",myPlyNumber,"Steel",myThickness/myPlyNumber,myAngle)
Create_Isotropic_Section_2D(myString,"Alu_section","Alu",myThickness)

Create_Pressure_Load(myString,"Cylinder-1","Explosion","Step-1","Outer_Surface",1)
